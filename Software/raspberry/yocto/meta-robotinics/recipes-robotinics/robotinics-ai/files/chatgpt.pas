...
uses
  ...
  fphttpclient, opensslsockets, aibase, aillmproviders,
...
begin
  ...
  FOwner.FResponse := FOwner.FResponse + UTF8Decode(FData);
  if Assigned(FOwner.FOnStreamData) then
    FOwner.FOnStreamData(FOwner, UTF8Decode(FData));
end;
...
begin
  if (FOwner = nil) or FOwner.FDestroying then
    Exit;
  FOwner.FResponse := UTF8Decode(FAnswer);
  ...
  if (not FSuccess) and (not FCancelled) and Assigned(FOwner.FOnRequestError) then
    FOwner.FOnRequestError(FOwner, UTF8Decode(FError));
...
procedure Register;
begin
  { Headless build: no Lazarus component palette registration. }
end;
...
begin
  ...
  try
    Obj.Add('provider', UTF8Encode(ProviderName));
    Obj.Add('model', UTF8Encode(GetModelName));
    Obj.Add('question_chars', Length(FQuestion));
    Obj.Add('streaming', FStreaming);
    if AITraceAllowsSensitiveContent(FTrace) then
      Obj.Add('question', UTF8Encode(FQuestion));
...
begin
  ...
  try
    ...
    if AITraceAllowsSensitiveContent(FTrace) then
      Obj.Add('response', UTF8Encode(FResponse));
...
begin
  ...
  FResponse := FResponse + UTF8Decode(AData);
  if Assigned(FOnStreamData) then
    FOnStreamData(Self, UTF8Decode(AData));
end;
              ...
              begin
                ...
                begin
                  ...
                  if (ContentData <> nil) and (ContentData.JSONType = jtString) then
                    Result := UTF8Decode(ContentData.AsString);
                end;
              end;
            end;
          end;
...
begin
  ...
  FLastURL := UTF8Decode(LProvider.BuildEndpoint(LConfig));
  FLastJSON := UTF8Decode(LProvider.BuildRequest(LConfig));
  BeginLLMTrace;
  ...
  try
    ...
    begin
      ...
      if Assigned(FOnRequestError) then
        FOnRequestError(Self, UTF8Decode(FLastError));
    end;
  finally
...
begin
  ...
  try
    ...
    if LModelStr <> '' then
      FCustomModel := UTF8Decode(LModelStr);
    FToken := UTF8Decode(Ini.ReadString('LLM', 'Token', Ini.ReadString('LLM', 'APIKey', UTF8Encode(FToken))));
    FURL := UTF8Decode(Ini.ReadString('LLM', 'URL', Ini.ReadString('LLM', 'Endpoint', UTF8Encode(FURL))));
...
begin
  ...
  try
    ...
    if FCustomModel <> '' then
      Ini.WriteString('LLM', 'Model', UTF8Encode(FCustomModel))
    else
      Ini.WriteString('LLM', 'Model', UTF8Encode(GetModelName));
    Ini.WriteString('LLM', 'Token', UTF8Encode(FToken));
    Ini.WriteString('LLM', 'URL', UTF8Encode(FURL));
